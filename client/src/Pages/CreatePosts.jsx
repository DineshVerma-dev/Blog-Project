...
function CreatePosts() {
  const location = useLocation();
  const navigate = useNavigate();
  const editingBlog = location.state || null;
  const [title, setTitle] = useState(editingBlog ? editingBlog.title : '');
  const [content, setContent] = useState(editingBlog ? editingBlog.content : '');
  const [blogs, setBlogs] = useState([]);
  const [isEditing, setIsEditing] = useState(!!editingBlog);
  ...
  return (
    <div className="max-w-11xl  mx-auto p-4 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100">
      <h1 className="text-3xl font-bold mb-4">{isEditing ? 'Edit Blog' : 'Create Blog'}</h1>
      <form onSubmit={handleSubmit}>
        <input
          type="text"
          ...
          onChange={handleTitleChange}
          placeholder="Title"
          className="w-full  max-w-5xl p-2 mb-4 border border-gray-400 dark:border-gray-600 rounded bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-100"
        />
        <Editor
          apiKey={import.meta.env.VITE_EDITOR_KEY}  
          value={content}
        ...
        <button
          type="submit"
          className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mt-4 transition-colors dark:bg-blue-700 dark:hover:bg-blue-600"
        >
          {isEditing ? 'Update' : 'Post'}
        </button>
      </form>
