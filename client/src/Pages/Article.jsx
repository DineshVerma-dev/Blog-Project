...
const Card = ({ title, content, image, link }) => (
  <div className="max-w-sm rounded overflow-hidden shadow-lg bg-white dark:bg-gray-800 m-4 transform hover:scale-105 transition-transform duration-300 ease-in-out">
    <img className="w-full h-48 object-cover" src={image} alt={title} />
    <div className="px-6 py-4">
    ...
    <div className="px-6 pt-4 pb-2">
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="inline-block bg-blue-500 dark:bg-blue-700 text-white dark:text-gray-200 rounded-full px-3 py-1 text-sm font-semibold hover:bg-blue-700 dark:hover:bg-blue-600"
      >
        Read More
      </a>
    </div>
...
function Article() {
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 p-8">
      <h1 className="text-3xl font-bold text-center mb-8 text-gray-900 dark:text-gray-100">Interesting Topics</h1>
      <div className="flex flex-wrap justify-center">
        {articleData.map((item, index) => (
          <Card key={index} title={item.title} content={item.content} image={item.image} link={item.link} />
        ))}
